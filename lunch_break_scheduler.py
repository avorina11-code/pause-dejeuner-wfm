...
# ---------------------------------------------------------------------------
def build_grid_html(df: pd.DataFrame, slot_labels: list) -> str:
    """
    Construit une grille visuelle HTML : une ligne par agent, une colonne
    par créneau horaire, avec la cellule correspondant à la pause de
    l'agent mise en évidence. Purement esthétique, complémentaire au
    tableau éditable (data_editor) déjà présent dans l'app.
    """
    rows_html = []
    for i, (_, row) in enumerate(df.sort_values(["Heure_debut", "Nom"]).iterrows()):
        creneau_agent = row["Creneau_pause"]
        stripe = "wfm-row-alt" if i % 2 else ""
        cells = []
        for label in slot_labels:
            if creneau_agent == label:
                cells.append('<td class="wfm-cell wfm-cell-active"><span class="wfm-dot">🍽️</span></td>')
            else:
                cells.append('<td class="wfm-cell"></td>')
        non_planifie_badge = (
            '<span class="wfm-badge wfm-badge-warn">Non planifié</span>'
            if creneau_agent == "Non planifié"
            else ""
        )
        rows_html.append(
            f'<tr class="{stripe}">'
            f'<td class="wfm-agent-cell">{row["Nom"]}<br>'
            f'<span class="wfm-sub">{row["Heure_debut"]} → {row["Heure_fin"]}</span>'
            f'{non_planifie_badge}</td>'
            + "".join(cells)
            + "</tr>"
        )

    header_cells = "".join(f'<th class="wfm-th">{label}</th>' for label in slot_labels)
    ...
    <style>
        ...
        .wfm-grid thead th:first-child {{
            text-align: left;
            border-radius: 14px 0 0 0;
        }}
        .wfm-grid thead th:last-child {{
    ...
    <div class="wfm-grid-wrapper">
        <table class="wfm-grid">
            <thead>
                <tr>
                    <th class="wfm-th">Agent</th>
                    {header_cells}
                </tr>
            </thead>
            <tbody>
...
if st.session_state.get("planning_genere"):
    ...
    grid_html = build_grid_html(df_edited, slot_labels_ordered)
