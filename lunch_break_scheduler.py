...
# ---------------------------------------------------------------------------
def build_grid_html(df: pd.DataFrame, slot_labels: list, capacite_max: int = None,
                     tolerance_depassement: int = 0) -> str:
    """
    Construit une grille visuelle HTML façon "tableau planning" :
    UNE COLONNE PAR CRÉNEAU HORAIRE (11:00-12:00, 12:00-13:00, ...) et,
    sous chaque en-tête, la liste des noms des agents affectés à ce
    créneau. Une colonne "Non planifié" est ajoutée à la fin s'il y a
    des agents sans créneau. Purement esthétique, complémentaire au
    tableau éditable (data_editor) déjà présent dans l'app.
    """
    all_labels = list(slot_labels)
    if (df["Creneau_pause"] == "Non planifié").any():
        all_labels = all_labels + ["Non planifié"]

    columns_html = []
    for label in all_labels:
        agents_slot = df[df["Creneau_pause"] == label].sort_values(["Heure_debut", "Nom"])
        count = len(agents_slot)

        if label == "Non planifié":
            header_class = "wfm-col-header wfm-col-header-warn"
        elif capacite_max is not None and count > capacite_max + tolerance_depassement:
            header_class = "wfm-col-header wfm-col-header-danger"
        elif capacite_max is not None and count > capacite_max:
            header_class = "wfm-col-header wfm-col-header-caution"
        else:
            header_class = "wfm-col-header wfm-col-header-ok"

        if count == 0:
            cards = '<div class="wfm-empty">— aucun agent —</div>'
        else:
            cards = "".join(
                f'<div class="wfm-card">'
                f'<span class="wfm-card-name">{row["Nom"]}</span>'
                f'<span class="wfm-card-time">{row["Heure_debut"]} → {row["Heure_fin"]}</span>'
                f'</div>'
                for _, row in agents_slot.iterrows()
            )

        columns_html.append(
            f'<div class="wfm-column">'
            f'<div class="{header_class}">{label}<span class="wfm-count">{count}</span></div>'
            f'<div class="wfm-column-body">{cards}</div>'
            f'</div>'
        )
    ...
    <style>
        ...
        .wfm-col-header-ok {{ background: #d4f7d4; }}
        .wfm-col-header-caution {{ background: #fff3cd; }}
        .wfm-col-header-danger {{ background: #f8d7da; }}
        .wfm-col-header-warn {{ background: #e2e8f0; color: #475569; }}
        .wfm-count {{
...
if st.session_state.get("planning_genere"):
    ...
    # ---------------------------------------------------------------------
    # 5bis. VUE GRILLE VISUELLE (Agents x Créneaux horaires)
    # ---------------------------------------------------------------------
    st.subheader("🗓️ Vue grille visuelle")
    st.caption(
        "Chaque ligne représente un agent, chaque colonne un créneau horaire. "
        "🍽️ indique le créneau de pause attribué."
    )
    grid_html = build_grid_html(df_edited, slot_labels_ordered, capacite_max, tolerance_depassement)
